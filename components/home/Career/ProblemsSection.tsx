export default function ProblemsSection() {
  ...
  return (
    <section className="relative bg-[#FFFFFF] xl:py-20 xl:px-32 lg:py-12 lg:px-10 md:px-10 md:py-12">
      {/* Gradient Blur Shape */}
      ...
      {/* Heading */}
      <h2 className="text-center xl:text-5xl lg:text-4xl md:text-3xl font-medium text-[#0048FF] mb-16">
        Are You Running Into These Problems
      </h2>

      {/* Cards */}
      <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 xl:gap-2 lg:gap-4 md:gap-2">
        {data.map((item) => (
          <div
            key={item.id}
            className="border border-[#0048FF] rounded-4xl xl:px-6 xl:py-8 lg:px-6 lg:py-8 md:px-4 md:py-4 bg-[#FFFFFF] hover:shadow-md transition"
          >
            {/* Number */}
            <h3 className="text-blue-600 xl:text-4xl lg:text-4xl md:text-3xl font-medium mb-4">
              {item.id}
            </h3>

            {/* Title */}
            <h4 className="font-medium xl:text-xl lg:text-lg md:text-base mb-2">
              {item.title}
            </h4>
