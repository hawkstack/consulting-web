...
export default function JobOpeningsSection() {
  ...
  return (
    <section className="w-full bg-[#FFFFFF] xl:py-18 xl:px-30 lg:py-16 lg:px-20 md:py-12 md:px-16">
      {/* Heading */}
      <div className="w-full mx-auto">
        <h2 className="xl:text-6xl lg:text-3xl md:text-2xl font-semibold text-[#0048FF]">
          Be part of HawkStack
        </h2>
        <p className="xl:mt-8 lg:mt-4 md:mt-4 text-[#2F2F2F] font-medium xl:max-w-[64%] xl:text-xl lg:text-lg md:text-md">
          At HawkStack, we put people first from the very beginning of your
          journey with us. Here’s how our hiring process works:
        </p>

        {/* Tabs */}
        <div className="flex flex-wrap gap-3 xl:mt-6 lg:mt-6 md:mt-6">
          {tabs.map((tab, i) => (
            <button
              key={i}
        ...
        {/* Job List */}
        <div className="xl:mt-10 lg:mt-6 md:mt-6 space-y-6 transition-all duration-300">
          {filteredJobs.map((job, i) => (
            ...
            >
              ...
              <div className="max-w-2xl">
                <h3 className="text-[#0048FF] font-semibold xl:text-xl lg:text-xl md:text-md">
                  {job.title}
                </h3>
                ...
                <div className="flex gap-3 mt-6">
                  <button className="flex items-center gap-2 font-medium xl:px-4 xl:py-1.5 lg:px-3 lg:py-1 md:px-2 md:py-1 border border-[#2B2B2B] rounded-full xl:text-sm md:text-xs text-black hover:bg-gray-100">
